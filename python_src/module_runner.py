...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Program for running Python helper scripts"
    )
    parser.add_argument(
        "--mint",
        action="store_true",
        help="Run program for filtering Mint transactions by month",
    )
    parser.add_argument(
        "--clean-downloads",
        "-cd",
        action="store_true",
        help="Run routine to clean Downloads directory",
    )
    return parser
