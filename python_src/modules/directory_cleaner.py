...
class DirectoryCleaner:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._target_dir = (
            target_dir if isinstance(target_dir, Path) else Path(target_dir).resolve()
        )
        ...
        logger_kwargs = dict(
            ...
            logs_basedir=kwargs.get(
                "logs_basedir", Path(__file__).resolve().parents[1].joinpath("logs")
            ),
    ...
    def _get_osx_date_added(content_path: Path) -> str:
        ...
        cp: sp.CompletedProcess = sp.run(
            ["mdls", "-name", "kMDItemDateAdded", "-raw", f"{content_path}"],
            check=True,
            stdout=sp.PIPE,
        )
